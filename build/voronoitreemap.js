...
},{"./d3.geom.nhull":1}],3:[function(require,module,exports){
require("./d3.geom.nhull");
...
module.exports = d3.layout.voronoiTreemap;



},{"./d3.geom.nhull":1}],4:[function(require,module,exports){
!function() {
  var d3 = {
    version: "3.5.3"
  };
...
},{}],"biojs-voronoitreemap":[function(require,module,exports){
require("./d3.geom.nhull");
require("./treemap");
require("./power");
...
voronoiTree.vtreemap = function(conf) {
  ...
  var json = conf.json,
      ...
      height = conf.height,
      sideid = conf.sideid,
      polygonid = conf.polygonid,
      sizeid = conf.sizeid,
      countid = conf.countid,
      chartid = conf.chartid,
      errorsid = conf.errorsid;
  ...
  d3.selectAll(sizeid).on("change", change);
  d3.selectAll(countid).on("change", change);
  d3.select(sideid).on("input", change);

  change();

  function change() {
    sides = +d3.select(sideid).property("value");
    if (sides > 12) sides = 120;
    d3.select(polygonid).call(smallPolygon, sides);
    treemap
        .polygon(regularPolygon(sides, width, height, 2 * ratio))
        .value(d3.select(sizeid).property("checked")
            ? function(d) { return d.size; }
            : function() { return 1; });
    stop = false;
  }
  
  var canvas = d3.select(chartid).append("canvas")
      .attr("width", width)
      .attr("height", height)
  ...
  function checkPolygon(p, nodes) {
    var res = [];
    for(i = 0; i < nodes.length; i++) {
      try{
      if(pointInPolygon(p, nodes[i].polygon)) {
          res.push(nodes[i].name);
      }}
      catch(err){console.log("Wait! Its Loading ... ")}
    }
    return res;
  }
  ...
  function getInfo(){
    ...
    var x = coordinates[0];
    var y = coordinates[1];
    var result = checkPolygon(coordinates,listOfNodes);
    console.log(result);

  }

  var svg = d3.select(errorsid).append("svg")
